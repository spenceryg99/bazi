...
class Geju {
  ...
  final String exampleDayMaster;
  final String exampleMonth;
  final String exampleStem;
  final String chengGe;
  final String poGe;
  final String xiangShen;
  final String meaning;
  const Geju({
    required this.name,
    required this.category,
    required this.shishen,
    required this.monthlyRule,
    required this.exampleDayMaster,
    required this.exampleMonth,
    required this.exampleStem,
    required this.chengGe,
    required this.poGe,
...
const zhengGe = <Geju>[
  Geju(
    name: '正官格', category: GejuCategory.zheng, shishen: '正官',
  ...
  Geju(
    name: '七杀格', category: GejuCategory.zheng, shishen: '七杀（偏官）',
  ...
  Geju(
    name: '正财格', category: GejuCategory.zheng, shishen: '正财',
  ...
  Geju(
    name: '偏财格', category: GejuCategory.zheng, shishen: '偏财',
    monthlyRule: '月令本气为「偏财」（我克之同性天干透出）。如甲木日主生于辰/戌月，戊土本气透干。',
    exampleDayMaster: '甲木', exampleMonth: '辰/戌', exampleStem: '戊土',
    chengGe: '喜食伤生、官星护。',
    poGe: '比劫夺财。',
    xiangShen: '官制比劫。',
    meaning: '慷慨大方、善经营、横财机遇。偏财为流动之财、投机之财、众人之财。',
  ),
  Geju(
    name: '正印格', category: GejuCategory.zheng, shishen: '正印',
  ...
  Geju(
    name: '偏印格', category: GejuCategory.zheng, shishen: '偏印（枭神）',
  ...
  Geju(
    name: '食神格', category: GejuCategory.zheng, shishen: '食神',
  ...
  Geju(
    name: '伤官格', category: GejuCategory.zheng, shishen: '伤官',
...
const waiGe = <Geju>[
  Geju(
    name: '从财格', category: GejuCategory.wai, shishen: '从',
  ...
  Geju(
    name: '从杀格', category: GejuCategory.wai, shishen: '从',
  ...
  Geju(
    name: '从儿格', category: GejuCategory.wai, shishen: '从',
  ...
  Geju(
    name: '化气格', category: GejuCategory.wai, shishen: '化',
  ...
  Geju(
    name: '专旺格（曲直格）', category: GejuCategory.wai, shishen: '专旺',
  ...
  Geju(
    name: '建禄格', category: GejuCategory.wai, shishen: '比劫',
...
List<Geju> get allGe => [...zhengGe, ...waiGe];

class BranchMainStem {
  final String stem;
  final String wuxing;
  const BranchMainStem(this.stem, this.wuxing);
}

const branchMainStem = <String, BranchMainStem>{
  '子': BranchMainStem('癸', '水'),
  '丑': BranchMainStem('己', '土'),
  '寅': BranchMainStem('甲', '木'),
  '卯': BranchMainStem('乙', '木'),
  '辰': BranchMainStem('戊', '土'),
  '巳': BranchMainStem('丙', '火'),
  '午': BranchMainStem('丁', '火'),
  '未': BranchMainStem('己', '土'),
  '申': BranchMainStem('庚', '金'),
  '酉': BranchMainStem('辛', '金'),
  '戌': BranchMainStem('戊', '土'),
  '亥': BranchMainStem('壬', '水'),
};

class ShishenGejuResult {
  final String shishen;
  final String geju;
  const ShishenGejuResult(this.shishen, this.geju);
}

ShishenGejuResult computeShishenGeju(String dmWx, String tgWx, bool sameYy) {
  const sheng = {'木': '火', '火': '土', '土': '金', '金': '水', '水': '木'};
  const ke = {'木': '土', '土': '水', '水': '火', '火': '金', '金': '木'};
  String rel;
  if (tgWx == dmWx) {
    rel = '比劫';
  } else if (sheng[tgWx] == dmWx) {
    rel = '印';
  } else if (sheng[dmWx] == tgWx) {
    rel = '食伤';
  } else if (ke[dmWx] == tgWx) {
    rel = '财';
  } else {
    rel = '官杀';
  }
  switch (rel) {
    case '官杀':
      return ShishenGejuResult(sameYy ? '七杀' : '正官', sameYy ? '七杀格' : '正官格');
    case '财':
      return ShishenGejuResult(sameYy ? '偏财' : '正财', sameYy ? '偏财格' : '正财格');
    case '印':
      return ShishenGejuResult(sameYy ? '偏印' : '正印', sameYy ? '偏印格' : '正印格');
    case '食伤':
      return ShishenGejuResult(sameYy ? '食神' : '伤官', sameYy ? '食神格' : '伤官格');
    default:
      return ShishenGejuResult(sameYy ? '比肩' : '劫财', '建禄/月劫格');
  }
}

const stemYinyang = <String, String>{
  '甲': '阳', '乙': '阴', '丙': '阳', '丁': '阴', '戊': '阳', '己': '阴',
  '庚': '阳', '辛': '阴', '壬': '阳', '癸': '阴',
};

const stemWuxing = <String, String>{
